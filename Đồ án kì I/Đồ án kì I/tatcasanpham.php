...
<body>
  ...
  ?>
	...
	<?php 
		$select = "select * from thong_tin_dien_thoai limit $dongbatdau,$kichthuoctrang";
		...
		<div class="container">
			<div class="row">
				<div class="col-mg-3 col-lg-3">
					...
					<div class="danhmuc">
						<i class="fas fa-caret-right"></i>
						 <h2 style="font-size: 15px;" class="title">NHÀ SẢN XUẤT</h2>
					</div>
					<div style="margin-bottom: 30px;" class="search_cost">
						<div class="widget">
							<fieldset>
								<?php 
									while ($h = mysqli_fetch_assoc($tt_sql)) {
								?>
							    <label style="width: 90%;text-align: left;" for="radio-<?=$h['ten_hang']?>"><?=$h['ten_hang']?></label>
							    <input value="<?=$h['ma_hang']?>" class="r" class="check" onclick="tk();" type="radio" name="radio-2" id="radio-<?=$h['ten_hang']?>"><br>
							    <?php 
								}
							    ?>
						  </fieldset>
	...
	<script type="text/javascript">
		function tk(){
			...
			var val2 = $('[name="radio-2"]:radio:checked').val();
			$.ajax({
				type: 'POST',
				dataType: 'text',
